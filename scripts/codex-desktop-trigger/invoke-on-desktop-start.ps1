...
foreach ($path in @($publisherPath, $RepositoryRoot, $CodexHome)) {
  if ($path.Contains('"')) {
...
Write-TriggerLog "ACTIVATE date=$dateText rrule=FREQ=MINUTELY;INTERVAL=1;COUNT=1 mode=desktop-automation publisher_pid=$($publisherProcess.Id)"
